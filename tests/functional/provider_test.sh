...
# provider provider_test_data_array
function test_get_data_from_provider_as_array() {
  local current_data="$1"
  local current_iteration=0

  echo "$(awk 'BEGIN{FS=OFS=""} {$1++} {print $1}' $_TEST_GET_DATA_FROM_PROVIDER_ITERATION_FILE)" > $_TEST_GET_DATA_FROM_PROVIDER_ITERATION_FILE
...
# provider provider_test_data_single_value
